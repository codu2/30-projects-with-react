...
class Router {
  ...
  init() {
    ...
    // hash change 될 때마다 this.checkRoute()를 실행시켜 줌
...
class Storage {
  saveTodo(id, todoContent) {
    // 새로 생성하는 todo 이므로 status는 값을 받지 않고 초기값으로 todo
...
class TodoList {
  ...
  onClickTodoList(event) {
    ...
    // btn 안의 icon을 클릭했을 때도 btn이 눌린 것처럼 인식하기 위해
    if (!btn) return;
