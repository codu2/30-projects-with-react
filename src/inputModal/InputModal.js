...
const InputModal = ({
  ...
  // idNum이 존재하면 edit, 존재하지 않으면 submit
  const edit = useCallback(
    ({ lectureName, day, startTime, endTime, lectureColor }) => {
      let valid = true;
      // 자신을 제외한 다른 것들과의 중복 체크
      for (let index = 0; index < timeTableData[day].length; index++) {
        if (
          checkOverLap(timeTableData[day][index], {
            start: startTime,
      ...
      // 수정하려는 data를 삭제한 상태로 전체 timetable data를 변경해줌
      const newTimeTableData = {
        ...timeTableData,
        [dayData]: filteredDayData,
      };

      // 수정하려는 day의 전체 값에 수정된 data 값을 넣어줌
      const newDayData = [
      ...
      // 수정하려는 data를 삭제한 timetable data와 수정하려는 data가 포함된 day의 값을 저장해줌
      setTimeTableData({ ...newTimeTableData, [day]: newDayData });
        ...
        <DialogContent style={{ width: "400px" }}>
          ...
          <Stack spacing={3} style={{ marginTop: "30px", width: "350px" }}>
            ...
            {errors.startTime?.type === "required" && (
              <p style={{ color: "#d32f2f" }}>강의 시작시간을 선택해주세요.</p>
            )}
            ...
            {errors.endTime?.type === "required" && (
              <p style={{ color: "#d32f2f" }}>강의 종료시간을 선택해주세요.</p>
            )}
            {errors.endTime?.type === "validate" && (
              <p style={{ color: "#d32f2f" }}>
                강의 시작시간과 종료시간을 확인해주세요.
              </p>
            )}
          </Stack>
